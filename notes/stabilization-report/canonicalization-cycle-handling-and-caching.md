...
However, the global cache by itself in insufficient to avoid exponential blowup and hangs when encountering cycles. To deal with this we also have [the `provisional_cache`](https://github.com/rust-lang/rust/blob/1a8fa555801329bd0e803d7384b5a21191c61f30/compiler/rustc_type_ir/src/search_graph/mod.rs#L612-L616) which is only used while inside of a trait solver cycle. This cache is allowed to be observable, which is fine, because we only move the cycle root to the global cache and the provisional cache entries for all other cycle participants just get dropped.

An ABA cycle can have different results from a BAB cycle, where `A` and `B` are distinct goals. We only cache the root of a cycle and track [`NestedGoals`](https://github.com/rust-lang/rust/blob/4e701dc6ba63a40c908a173df269a55e1fd6297e/compiler/rustc_type_ir/src/search_graph/mod.rs#L498-L514) of each global cache entry. We then don't use global cache entries, if one of these nested goals is currently on the stack or in the provisional cache: [source](https://github.com/rust-lang/rust/blob/4e701dc6ba63a40c908a173df269a55e1fd6297e/compiler/rustc_type_ir/src/search_graph/mod.rs#L1174-L1242).
